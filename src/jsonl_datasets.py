...
def make_train_dataset(args, tokenizer, accelerator=None):
    ...
    train_transforms_condition = transforms.Compose(
        [
            transforms.ToTensor(),
            transforms.Normalize([0.5], [0.5]),
        ]
    ...
    def preprocess_train(examples):
        ...
        _examples["cond_pixel_values"] = [train_transforms_condition(source) for source in source_images]
        ...
        if "position" not in column_names:
            raise ValueError('position not in column_names')
        #_examples["position"] = [position_example for position_example in examples['position']]
        _examples["position"] = examples["position"]
...
def resize_and_pad(image_tensor, target_height=192):
    _, h, w = image_tensor.shape

    if h < target_height:
        padding_height = target_height - h
        # white background padding
        white_padding = (torch.ones((3, padding_height, w)) * 1.0).to(image_tensor.device, dtype=image_tensor.dtype)  # 白色是填充值 1.0
        image_tensor = torch.cat((image_tensor, white_padding), dim=1)
...
def collate_fn(examples):
    ...
    #cond_pixel_values = torch.stack([resize_and_pad(example["cond_pixel_values"]) for example in examples])
    cond_pixel_values = torch.stack([example["cond_pixel_values"] for example in examples])
    cond_pixel_values = cond_pixel_values.to(memory_format=torch.contiguous_format).float()
    ...
    token_ids_clip = torch.stack([
        example["token_ids_clip"] if isinstance(example["token_ids_clip"], torch.Tensor)
    ...
    token_ids_t5 = torch.stack([
        example["token_ids_t5"] if isinstance(example["token_ids_t5"], torch.Tensor)
