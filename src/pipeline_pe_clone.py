...
def prepare_img_ids_new(batch_size: int, packed_latent_height_layout: int, packed_latent_width_layout: int,packed_latent_height_img:int, packed_latent_width_img:int,position_lists: list):
    ...
    for position_list in position_lists:
        ...
        if random.random() < 0.25:  # 25% for layout free
            ...
            #img_ids_layout[..., 2] += packed_latent_width_img
            img_ids_np_layout = img_ids_layout.numpy()
            ...
            for src_pts_list, dst_pts_2_list in position_list:
                ...
                if len(src_pts)==8:
                    width = dst_bottom_right[0] - dst_top_left[0]
                    height = dst_bottom_right[1] - dst_top_left[1]
                    dst_pts_2 = np.array([
                        [dst_top_left[0], dst_top_left[1]],
                        [dst_top_left[0] + width // 3, dst_top_left[1]],
                        [dst_top_left[0] + 2 * width // 3, dst_top_left[1]],
                        [dst_bottom_right[0], dst_top_left[1]],
                        [dst_bottom_right[0], dst_bottom_right[1]],
                        [dst_top_left[0] + 2 * width // 3, dst_bottom_right[1]],
                        [dst_top_left[0] + width // 3, dst_bottom_right[1]],
...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def prepare_latents(
        ...
        #position_file = 'autodl-tmp/photo-data/data-5000-sample/data_condition_sample/126758-position.txt'
