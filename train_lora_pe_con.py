...
from src.pipeline_pe_clone import FluxPipeline, position_encoding_clone,prepare_img_ids_new
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                vae_scale_factor = 16
                height_ = 2 * (int(pixel_values.shape[-2]) // vae_scale_factor)#512//16*2
                width_ = 2 * (int(pixel_values.shape[-1]) // vae_scale_factor)

                height_condition = 2 * (int(cond_pixel_values.shape[-2]) // vae_scale_factor)
                width_condition = 2 * (int(cond_pixel_values.shape[-1]) // vae_scale_factor)

                latent_image_ids = prepare_img_ids_new(cond_input.shape[0],height_condition//2,width_condition//2,height_//2,width_//2,batch["position"])
            ...
            if accelerator.is_main_process:
                if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                    ...
                    target_size = 1024
                    def process_condition_image2(image_path: str) -> Image.Image:
                        condition_image = Image.open(image_path).convert("RGB")
                        return condition_image
                    condition_image = process_condition_image2(args.validation_image)
